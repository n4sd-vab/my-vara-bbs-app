...
let messageListMode = "local";
let listBuffer = "";
let endOfFileList = false;
...
ipcMain.on("yapp:send-start", (event, info) => {
  setInYappSend(true);
  yappSend = null;
  console.log("IPC: yapp:send-start received", { stack: new Error().stack });
...
// ============================================================================
// IPC HANDLERS - VARA CONNECTION
// ============================================================================

ipcMain.handle("vara-connect", async () => {
  return setupVaraConnection();
});
...
function setupVaraConnection() {
  return new Promise((resolve, reject) => {
    ...
    dataSocket.on('data', handleDataSocketInput);
...
// ============================================================================
// VARA/BBS LOGIC - CONNECTION MANAGEMENT
// ============================================================================
...
// ============================================================================
// VARA/BBS LOGIC - DATA SOCKET HANDLER
// ============================================================================

function handleDataSocketInput(data) {
  // Detect BBS prompt
  ...
  try {
    if (inYappSend && yappSend) {
      ...
      if (bytes[0] === 0x15 || bytes[0] === 0x18) {
        const message = text.slice(2).split('\r')[0];
    ...
    // READ MODE (message body retrieval)
    if (!inReadMode) {
      const text = data.toString();
      readBuffer += text;
...
// ============================================================================
// HELPER FUNCTIONS - SETTINGS & DATABASE
// ============================================================================
...
// ============================================================================
// HELPER FUNCTIONS - ASYNC WAIT FUNCTIONS
// ============================================================================
...
// ============================================================================
// HELPER FUNCTIONS - FILE LIST & YAPP UTILITIES
// ============================================================================
